...
def preprocess_data(data, output_dir):
    ...
    # Subsample the dataset (optional: reduces size significantly)
    data = data.iloc[::10]  # Keep every 10th row

    # Rescale the data using MinMaxScaler
    scaler = MinMaxScaler()
    scaled_data = scaler.fit_transform(data[['Open', 'High', 'Low', 'Close', 'Volume_(BTC)']]).astype(np.float32)

    # Create sequences
    X, y = [], []
    seq_length = 60  # Reduced from 1440 to 60 (1 hour of past data)
